...
module.exports.startUploadPlaylist = async (mainWindow, link, playlist, client, clientUid, session) => {
    try {
        return await SpottyDL.getPlaylist(link)
            .then(async(results) => {
                let playlistSpoty;
                let playlistData;
                ...
                playlistSpoty = await SpottyDL.downloadPlaylist(results, path.join(__dirname, '../output'), true);

                if (playlist == 'new') {
                  const newList = await createPlaylist(playlistData, client, clientUid, session);
                  playlist = newList.kind;
                }
        
                await playlistSpoty.forEach(async play => {
                    if (play.status == 'Success') {
                        filename = play.filename.replace(path.join(__dirname, '../output') + '/', "");
...
async function createPlaylist(playlistData, client, clientUid, session) {
    ...
    const newPlaylist = await client.playlists.createPlaylist(client, {title: playlistName, 'visibility': 'public'}).then(async (data) => {
        ...
        await downloadImage(playlistData.playlistCoverURL);
